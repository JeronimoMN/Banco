...
export class TransferFormComponent {
  ...
  onSubmit(){
      this.clientService.getTransaccion({'cuenta': this.transferForm.value.cuenta?.split(/\s(.+)/)[0], 'cuentaDestino': this.transferForm.value.cuentaDestino, 'monto': this.transferForm.value.monto}).subscribe(
        (response: any)=> {
          if(response == "Valido"){
            Swal.fire({
